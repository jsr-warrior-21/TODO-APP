...
const Dashboard = ({ onLogout, darkMode, setDarkMode }) => {
  ...
  const getSortedTasks = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "priority":
        return sorted.sort(
          (a, b) => (b.is_important ? 1 : 0) - (a.is_important ? 1 : 0),
        );
      case "alphabetical":
  ...
  const handleToggleImportant = async (taskId) => {
    try {
      ...
      const updatedTasks = tasks.map((t) =>
        t.id === taskId ? { ...t, is_important: !t.is_important } : t,
      );
      setTasks(updatedTasks);
      await markImportant(taskId);
      const action = task?.is_important
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      {/* Fixed Activity Log Dropdown - Properly positioned for mobile */}
      {showActivity && (
        <>
          <div
            className="fixed inset-0 z-40"
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -20, scale: 0.95 }}
            transition={{ duration: 0.2 }}
            className="fixed left-4 right-4 sm:right-4 sm:left-auto top-20 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 p-4 sm:w-96 z-50 max-h-[70vh] overflow-y-auto transition-colors duration-300"
          >
            ...
            {activityLog.length === 0 ? (
              ...
              <div className="space-y-2">
                {activityLog.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-start gap-2 p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition"
                  >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
          {[
            ...
            <motion.div
              ...
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-4 sm:p-5 border border-gray-100 dark:border-gray-700 group cursor-pointer"
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${stat.color} rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 transition flex-shrink-0`}
                >
        ...
        {selectedTasks.length > 0 && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-blue-100 dark:border-blue-900/30 p-3 sm:p-4 mb-4"
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700"
                >
                  Clear
                </button>
              </div>
              ...
              <AnimatePresence>
                {filteredTasks.map((task) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -50 }}
                    transition={{ duration: 0.2 }}
                    className={`group bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-4 sm:p-5 border border-gray-100 dark:border-gray-700 ${
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        >
                          {task.completed ? (
                            <div className="w-5 h-5 sm:w-6 sm:h-6 bg-green-500 rounded-full flex items-center justify-center shadow-md shadow-green-500/30">
                              <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                            </div>
                          ) : (
                            <Circle className="w-5 h-5 sm:w-6 sm:h-6 text-gray-300 dark:text-gray-600 hover:text-blue-500 transition" />
                          )}
                        </button>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-1 sm:gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5">
                              {task.is_important && (
                                <span className="inline-flex items-center gap-0.5 text-[10px] sm:text-xs px-2 py-0.5 bg-gradient-to-r from-red-100 to-pink-100 dark:from-red-900/30 dark:to-pink-900/30 text-red-600 dark:text-red-400 rounded-full font-medium whitespace-nowrap">
                                  <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-red-500" />
                                  Important
                                </span>
                              )}
                              {task.due_date && (
                                <span
                                  className={`inline-flex items-center gap-0.5 text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${getDueDateColor(task.due_date)}`}
                                >
                              ...
                              {task.due_date &&
                                ...
                                !task.completed && (
                                  <span className="inline-flex items-center gap-0.5 text-[10px] sm:text-xs px-2 py-0.5 bg-gradient-to-r from-red-100 to-red-200 dark:from-red-900/30 dark:to-red-800/30 text-red-600 dark:text-red-400 rounded-full font-medium whitespace-nowrap animate-pulse">
                          ...
                          {task.description && (
                            <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm mt-1.5 break-words">
                              {task.description}
                            </p>
                          )}
                      ...
                      <div className="flex items-center gap-1 sm:gap-1.5 ml-auto sm:ml-0 flex-shrink-0">
                        <button
                        ...
                        >
                          <Star
                            className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${task.is_important ? "fill-yellow-500" : ""}`}
                          />
                        </button>
                        <button
                          onClick={() => {
                          ...
                          className="p-1.5 sm:p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition"
                        ...
                        <button
                          ...
                          className="p-1.5 sm:p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 transition"
